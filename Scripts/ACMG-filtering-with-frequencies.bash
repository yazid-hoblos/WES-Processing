...
for file in $source/*;do  
   ...
   if grep -q "hem" $file;then ((males++));else ((females++));fi
   ((patients++))
...
tail -n +2 "$target/whole_data.csv" | sort -f -t',' -k7 > "$target/temp.csv"

file=''
reduced_file=''
echo "variants,gene,chromosome,pHGVS,details,patients,numPatients,num_hom,num_het,num_hem,frequency,consensus,$(head -n 1 "$target/whole_data.csv" | cut -d ',' -f13-)" > "$target/variants.csv"
echo "variants,gene,chromosome,pHGVS,details,patients,numPatients,num_hom,num_het,num_hem,frequency,consensus,$(head -n 1 "$target/whole_data.csv" | cut -d ',' -f13-)" > "$target/reduced_variants.csv"
...
tail -n +2 "$target/temp.csv" | awk -v target=$target -v initiated=$initiated -v variant=$variant -v all_patients="$all_patients" -v reduced_all_patients="$reduced_all_patients" -v variant_details="$variant_details" -v other_details="$other_details" -v file="$file" -v reduced_file="$reduced_file" -v num_patients=$num_patients -v num_hom=$num_hom -v num_het=$num_het -v num_hem=$num_hem -v reduced_num_patients=$reduced_num_patients -v reduced_num_hom=$reduced_num_hom -v reduced_num_het=$reduced_num_het -v reduced_num_hem=$reduced_num_hem -v total_alleles=$total_alleles -v chrom=$chrom -v chr_X_alleles=$chr_X_alleles -v chr_Y_alleles=$chr_Y_alleles -F',' '{
  if ($7 == variant) {
    ...
    if ($6 >= 0.2 && $12 >= 15){
        ...
        file = file variant "," variant_details "," all_patients "," num_patients "," num_hom "," num_het "," num_hem "," freq "," other_details "\n"
        ...
        if (initiated==1){ 
            ...
            reduced_file = reduced_file variant "," variant_details "," reduced_all_patients "," reduced_num_patients "," reduced_num_hom "," reduced_num_het "," reduced_num_hem "," reduced_freq "," other_details "\n" 
        }
        if ($6 >= 0.2 && $12 >= 15){    
            reduced_all_patients = $1 " " $6 " " $12 " " $10
            reduced_num_patients=1
